...
export const Route = createFileRoute("/(main)/settings/role-management")({
...
const data: RoleManagementData[] = [
  ...
  },
  {
    ...
    name: "Developer",
    associated_users: 16,
    tags: ["Frontend", "User Interface Design"],
    status: "Active",
  },
];
...
function RouteComponent() {
  ...
  const columns: ColumnDef<RoleManagementData>[] = [
    {
      id: "select",
      header: ({ table }) => (
        <Checkbox.Root
    ...
    },
    {
      ...
      header: ({ column }) => (
        <div className="flex items-center gap-0.5">
          <span className="text-paragraph-sm text-text-sub-600">Tags</span>
          <button
            type="button"
            onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
          >
      ...
      <Header
        icon={
          <div className="flex size-12 shrink-0 items-center justify-center rounded-full bg-bg-white-0 shadow-regular-xs ring-1 ring-inset ring-stroke-soft-200">
            <RiShieldLine className="size-6 text-text-sub-600" />
          </div>
        }
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
            <Input.Root size="small" className="w-full sm:w-auto">
              <Input.Wrapper>
                ...
                <Input.Input placeholder="Search Role" />
                <Kbd.Root>⌘1</Kbd.Root>
              </Input.Wrapper>
            </Input.Root>
