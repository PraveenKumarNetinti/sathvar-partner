...
function VerticalStepperArrow<T extends React.ElementType>({
  ...
  return (
    <Component
      className={cnExt("text-text-sub-600 size-5 shrink-0", className)}
      {...rest}
    />
  );
}
...
const verticalStepperItemVariants = tv({
  slots: {
    root: [
      // base
      "rounded-10 text-paragraph-sm grid w-full auto-cols-auto grid-flow-col grid-cols-[auto_minmax(0,1fr)] items-center gap-2.5 p-2 text-left",
    ],
    indicator: [
      // base
      "text-label-xs flex size-5 shrink-0 items-center justify-center rounded-full",
    ],
  },
  variants: {
    state: {
