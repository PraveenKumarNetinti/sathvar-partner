...
function ColorThumb({ className, ...props }: AriaColorThumbProps) {
  return (
    <AriaColorThumb
      className={composeRenderProps(className, (className) =>
        cn("ring-stroke-white-0 z-50 size-3 rounded-full ring-2", className),
      )}
      {...props}
    />
  );
...
function ColorSwatchPickerItem({
  ...
  return (
    <AriaColorSwatchPickerItem
      className={composeRenderProps(className, (className) =>
        cn(
          "group/swatch-item cursor-pointer p-1 focus:outline-hidden",
          className,
        ),
      )}
      {...props}
...
function ColorSwatch({ className, style, ...props }: AriaColorSwatchProps) {
  return (
    <AriaColorSwatch
      className={composeRenderProps(className, (className) =>
        cn(
          "border-stroke-white-0 size-4 rounded-full group-data-[selected=true]/swatch-item:border-2 group-data-[selected=true]/swatch-item:ring-[1.5px]",
          className,
        ),
      )}
      style={({ defaultStyle }) => ({
