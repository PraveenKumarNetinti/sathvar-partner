...
>(({ className, ...rest }, ref) => {
  return (
    <ScrollArea.Scrollbar
      ref={ref}
      className={cn(
        "relative z-30 flex h-5 touch-none select-none justify-center border-t border-stroke-soft-200 bg-bg-white-0 py-1.5",
        className,
      )}
      orientation="horizontal"
      {...rest}
    >
      <ScrollArea.Thumb className="!h-1 shrink-0 rounded-full bg-stroke-soft-200" />
    </ScrollArea.Scrollbar>
  );
});
