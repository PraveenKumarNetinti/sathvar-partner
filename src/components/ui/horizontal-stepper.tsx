...
function HorizontalStepperSeparatorIcon<T extends React.ElementType>({
  ...
  return (
    <Component
      className={cnExt("text-text-soft-400 size-5 shrink-0", className)}
      {...rest}
    />
  );
}
...
const horizontalStepperItemVariants = tv({
  slots: {
    root: [
      // base
      "text-paragraph-sm flex items-center gap-2",
    ],
    indicator: [
      // base
  ...
  variants: {
    state: {
      ...
      default: {
        ...
        indicator:
          "bg-bg-white-0 text-text-sub-600 ring-stroke-soft-200 ring-1 ring-inset",
      },
    },
  },
  defaultVariants: {
